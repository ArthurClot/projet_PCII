...
public class Affichage extends JPanel {
	...
	private static boolean flagDeDebut=false;//sauf a l'initialisation, il est a vrai quand la partie recommence activee par la classe Controller
	...
	/** dimensions de la fenetre et de l'horizon*/
	...
	/**methode transformant l'etat de la partie
	 * une methode utilisee par la classe Controls et Avancer
	 */
	...
	/**
	 * methode qui dessine le vehicule enfonction de la fleche directionnelle appuyee
	 * @param g
	 */
	...
	private void dessineRoad(Graphics g) {
		...
		for(int i=0;i<this.road.getLigneDroite().size()-1;i++) {//entre chaques paire de points ( d'arraylist ligne on remplie les tableaux)
			...
			g.setColor(Color.yellow);	//on choisit la couleur des ovales formant la route 
	...
	/**
	 * methode qui dessine/affiche le temps a la gauche en haut de la fenetre
	 * @param g
	 */
	private void dessineMinuteur(Graphics g) {

		g.setColor(Color.red);//choix de la couleur des lettres et chiffres du score
		g.setFont(new Font( "Cambria" ,Font.BOLD,40));// choix de la police et de la taille des lettres et chiffres du score
		g.drawString("Time : "+etat.getMinuteur(), 150, HAUT_FENETRE/11);//on place et dessine un score qui s'incremente a chaque depassement de points
	}

	/**
	 * methode qui dessine/affiche le temps final au centre de la fenetre de menu
	 * @param g
	 */
	 ...
	 */
	/**
	 * methode qui dessine/affiche un message de fin, un message clignotant "start"
	 *  avec le score final (appel de la methode dessineTempsFinal(Graphics g))
	 * @param g
	 */
	private void MenuFin(Graphics g) {
		...
		g.setFont(new Font( "Cambria" ,Font.ITALIC,32));// choix de la police et de la taille de l'indication "start"
		g.drawString("------------------>  press SPACE to play  <------------------",  (LARG_FENETRE/2)-305, HAUT_FENETRE-100);
	}
	/**
	 * methode utilisee par la classe controls pour (re)demarrer la partie.
	 */
