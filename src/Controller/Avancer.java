...
public class Avancer implements Runnable {
	...
	private void variationSpeed() {
		...
		if (etat.testRalentissement()==false) { //on va plus vite
			...
			this.time-=(this.time/TIMEMAX)/10; //le "/10" cest pour que temporiser l'acceleration)
			...
			this.time+=(this.time/TIMEMAX)/2;//le "/2" cest pour que temporiser un peu la decceleration)
